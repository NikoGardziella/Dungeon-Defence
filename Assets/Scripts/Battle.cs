...
namespace DungeonDefence
{
	public class Battle
	{
		...
		public BattlePlayer MainPlayer = new BattlePlayer();
		public List<Unit> _units = new List<Unit>();
		public List<Spell> _spells = new List<Spell>();
		public List<Unit> _dungeonunits = new List<Unit>();
		...
		private void AssignTargetPlayer(int index)
		{
			var path = GetPathToPlayer(index);
			if (path.Item1 >= 0)
			{
				_dungeonunits[index].AssignTarget(path.Item1, path.Item2);
			}
		}

		//For now only pth to player
		...
		//	BattleVector2Int playerGridPosition = WorldToGridPosition(MainPlayer.position);
			...
			List<int> columns = new List<int>();
			List<int> rows = new List<int>();
			int startX = (int)MainPlayer.position.x;
			int endX = startX + 1;
			int startY = (int)MainPlayer.position.y;
			int endY = startY + 1;
				...
				for (int x = 0; x < columns.Count; x++)
				{
					for (int y = 0; y < rows.Count; y++)
					{
						if (x >= 0 && y >= 0 && x < Data.gridSize && y < Data.gridSize)
						{
							Path path1 = new Path();
							Path path2 = new Path();
							path1.Create(ref search, unitGridPosition, new BattleVector2Int((int)MainPlayer.position.x , (int)MainPlayer.position.y));
							path2.Create(ref unlimitedSearch,unitGridPosition, new BattleVector2Int((int)MainPlayer.position.x , (int)MainPlayer.position.y));
							pubPath1 = path1;
							pubPath2 = path2;
							if (path1.points != null && path1.points.Count > 0)
							{
