namespace DungeonDefence
{
	...
	public class UI_Battle : MonoBehaviour
	{
		...
		[SerializeField] public TextMeshProUGUI _lootDarkElixirText = null;
		 ...
		 public bool Display(List<Data.Building> buildings, long defender, Data.BattleType battleType)
    {
		...
		private void UpdateLoots()
		{
			...
			_lootDarkElixirText.text = looted.Item3 + "/" + looted.Item6;
		}
					...
					if (frame > battle.frameCount)
					{
						if (toAdd.Count > 0)
						{
							...
							for (int i = toAdd.Count - 1; i >= 0; i--)
							{
								...
								{
									if (Player.instance.data.units[j].databaseID == toAdd[i].id)
									{
										battle.AddUnit(Player.instance.data.units[j], toAdd[i].x, toAdd[i].y, UnitSpawnCallBack, UnitAttackCallBack, UnitDiedCallBack, UnitDamageCallBack, UnitHealCallBack);
