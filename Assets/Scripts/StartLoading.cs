        ...
        private void RecoverConfirm()
        {
            ...
            else if (waitingForCode)
            {
                ...
                if (!string.IsNullOrEmpty(code))
                {
                    ...
                    packet.Write((int)Player.RequestId.CONFIRMCODE);
            ...
            else
            {
                ...
                if (!string.IsNullOrEmpty(email) && Data.IsEmailValid(email))
                {
                    ...
                    _buttonCancel.interactable = false;
                    Packet packet = new Packet();
                    packet.Write((int)Player.RequestId.SENDCODE);
                    string device = SystemInfo.deviceUniqueIdentifier;
                    packet.Write(device);
                    packet.Write(email);
                    Debug.Log(email);
